...
* [FLExText Editor Suite](https://flextext.app/) - An app suite designed to make recording, audio-segmenting, transcribing, and glossing fast and easy. The Researcher Panel app is a system delegating the initial phases of this work to native speakers on their own devices and getting the results back. Results export to ELAN, SayMore, FLEx, Lameta, and more. An Audio Segmentation app makes it easy to take audio recordings and interlinear text lines and match them up for use with ELAN or other such tools.
